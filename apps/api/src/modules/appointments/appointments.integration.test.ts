...
describe.skipIf(!run)('appointments integration (create, conflict, idempotency, tenant)', () => {
  ...
  beforeAll(async () => {
    ...
    // Quarta-feira (weekday=3 em business_hours); alinhado ao relógio fake para não cair no passado no CI.
  ...
  it('cria agendamento awaiting_confirmation com confirmação explícita', async () => {
    const row = await createAppointment(
      tenantA,
      {
        ...
        service_id: serviceA,
        starts_at: '2026-05-13T15:00:00.000Z',
        ends_at: '2026-05-13T15:30:00.000Z',
        source: 'api',
      ...
      await withAppTenant(c, tenantA, async () => {
        await c.query(
          `INSERT INTO appointments (
             ...
             '2026-05-13T16:00:00.000Z','2026-05-13T16:30:00.000Z',
             'confirmed','api',$6)`,
    ...
    await expect(
      createAppointment(
        tenantA,
        {
          ...
          professional_id: profA,
          service_id: serviceA,
          starts_at: '2026-05-13T16:15:00.000Z',
          ends_at: '2026-05-13T16:45:00.000Z',
          source: 'api',
    ...
    await expect(
      createAppointment(
        tenantA,
        {
          ...
          professional_id: profA,
          service_id: serviceA,
          starts_at: '2026-05-13T11:00:00.000Z',
          ends_at: '2026-05-13T11:30:00.000Z',
          source: 'api',
          idempotency_key: key,
          explicit_confirmation: true,
        },
    ...
    await expect(
      createAppointment(
        tenantA,
        {
          ...
          professional_id: profA,
          service_id: serviceA,
          starts_at: '2026-05-13T16:00:00.000Z',
          ends_at: '2026-05-13T16:20:00.000Z',
          source: 'api',
